...
class TrainLoop:
    ...
    def run_loop(self):
        while (
            ...
            cond = {k: v for k, v in cond.items() if k =="y"}
        ...
        if (self.step - 1) % self.save_interval != 0:
            self.save()
            if self.save_img or self.save_video:
                self.make_plots(batch, model_kwargs=cond)
    ...
    def make_plots(self, batch, model_kwargs):
        if dist.get_rank() != 0:
            return
        with th.no_grad():
            samples = []
            for sample in self.diffusion.sample_from_img_progressive(
                self.model, batch, model_kwargs=model_kwargs
            ):
